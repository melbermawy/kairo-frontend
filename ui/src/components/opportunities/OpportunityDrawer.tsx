...
export function OpportunityDrawer({
  ...
  const existingPackageId = null; // Would be fetched from mockApi
  const packagesLink = existingPackageId
    ? `/brands/${brandId}/packages/${existingPackageId}`
  ...
  const drawerContent = (
    <>
      ...
      <div className="flex-shrink-0 px-5 py-4 border-t border-kairo-border-subtle bg-kairo-bg-panel">
        <Link href={packagesLink} className="block">
          <KButton className="w-full justify-center" data-testid="generate-package-btn">
            <svg
        ...
        <p className="text-[11px] text-kairo-fg-subtle text-center mt-2">
          {existingPackageId
            ? "View existing package"
            : "Create a new content package from this opportunity"}
        </p>
      </div>
    </>
  );
